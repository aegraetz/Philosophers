...
/*   Updated: 2022/10/24 13:49:19 by agraetz          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*Initialise the struct containing the timings of the 
process. Also triggers the initialisation of the 
philosophers struct and initialises the mutex locks for 
the forks. Returns 0 if anything doesn't initiliase */
...
/* Initialise the mutex locks for forks, status reporting 
and stopping the sim */
static bool	init_mutex(t_process *process)
{
...
/*ensures deadlocks are avoided by having the even 
numbered philosophers taking the fork on the left 
first, forcing the philosopher on the left to wait 
till both forks are free and therefore ensuring 
they all take turns eating*/
static void	alloc_forks(t_phil *phil)
{
	phil->fork[0] = phil->id;
