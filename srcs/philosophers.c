...
void	*philosophise(void *data)
{
	...
	delay_start(phil->process->start_time); //delay stops conflict between threads
	if (phil->process->t_2_die == 0)
		return (0);
	if (phil->process->no_phils == 1)//separate process for a single philosopher
		return (one_phil(phil));
	else if (phil->id % 2) //even num philosophers start by thinking
		think_time(phil, true);
	while (sim_stopped(phil->process) == false)
	{
		eat_n_sleep(phil); //odd num philosophers get straight into eating
		think_time(phil, false);
	}
	return(0);
}

/*lonely philosopher only has 1 fork and can't eat. Picks up fork and then waits t_2_die.
...
/*when it's time to eat, philosopher will wait for forks and then eat for set amount of time.
When meal time over, philosopher sleeps and fork mutexes are unlocked for others to use*/
static void	eat_n_sleep(t_phil *phil)
{
	pthread_mutex_lock(&phil->process->fork_locks[phil->fork[0]]);
	status_report(phil, false, GOT_FORK_1);
